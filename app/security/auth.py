...
def get_current_user(credentials: HTTPAuthorizationCredentials | None = Depends(security),db: Session = Depends(get_db),) -> User:
    """Authenticate the request and return its user; this does not check roles."""
...
require_authenticated_user = require_roles(UserRole.ADMIN, UserRole.USER, UserRole.GUEST)
