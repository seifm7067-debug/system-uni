...
| **Fencing for stale workers** | Failure transitions (`failed`/`queued`-retry) are guarded by `(worker_token, job_version)`: a worker that lost its lease cannot fail or requeue a job owned by another. Completion is arbitrated by the `UNIQUE(report.job_id)` row: whichever execution finishes first persists the report and finalizes the job; late workers collide on the constraint, adopt the winner's report, and never regenerate. The heartbeat watches for a persisted report and stops the losing execution early. |
