...
def upgrade() -> None:
    ...
    op.alter_column('course', 'department_id',
               ...
               nullable=True)
    op.add_column('enrollment', sa.Column('enrollment_code', sa.String(length=50), nullable=False))
    op.drop_column('enrollment', 'cenrollment_code')


def downgrade() -> None:
    """Downgrade schema."""
    op.add_column('enrollment', sa.Column('cenrollment_code', sa.VARCHAR(length=50), autoincrement=False, nullable=False))
