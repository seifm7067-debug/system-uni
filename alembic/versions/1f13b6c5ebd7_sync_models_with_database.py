...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.UniqueConstraint(
            "student_id", "course_offering_id", name="uq_enrollment_student_offering"
        ),
    )
    op.add_column(
        "course_offering",
    ...
    op.add_column(
        "course_schedule", sa.Column("day", sa.String(length=10), nullable=False)
    )
    op.add_column(
        "course_schedule",
    ...
    op.create_unique_constraint(
        "uq_course_schedule",
        "course_schedule",
        ["course_offering_id", "day", "start_time", "end_time"],
    )
...
def downgrade() -> None:
    ...
    op.add_column(
        "course_schedule",
        sa.Column(
            "day_of_week", sa.VARCHAR(length=15), autoincrement=False, nullable=False
        ),
    )
